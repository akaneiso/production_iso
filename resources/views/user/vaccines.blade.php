...
<div class="row row-cols-1 row-cols-md-3 g-4">
  ...
  <div class="card text-left">
    ...
    <div class="card-body">
      ...
      @if($age<=2) <h5 class="card-text">次回の接種推奨日：{{ $birthdayPlusTwoMonths2->format('Y年m月d日') }} </h5>
        <h5>ワクチン名：{{$vaccine2->name}}</h5>
      <div class="form-group">
                <input type="submit" name="type" value="接種完了" @if($vaccine2->type) checked @endif>
            </div>
        @endif
        @endforeach

        @foreach($vaccines5 as $vaccine5)
        @if($age>2 && $age<=5) <h5 class="card-text">次回の接種推奨日：{{ $birthdayPlusTwoMonths5->format('Y年m月d日') }} </h5>
          <h5>ワクチン名：{{$vaccine5->name}}</h5>
        <div class="form-group">
        <input type="submit" name="type" value="接種完了" @if($vaccine5->type) checked @endif>
          ...
          @if($age>5 && $age<=12) <h5 class="card-text">次回の接種推奨日：{{ $birthdayPlusTwoMonths12->format('Y年m月d日') }}</h5>
            <h5>ワクチン名：{{$vaccine12->name}}</h5>
          <div class="form-group">
                <input type="submit" name="type" value="接種完了" @if($vaccine12->type) checked @endif>
            </div>
            @endif
            @endforeach
          ...
          <div class="form-group">
                <input type="submit" name="type" value="接種完了" @if($vaccine36->type) checked @endif>
            </div>
            @endif
            @endforeach
    </div>
